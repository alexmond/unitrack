...
class JUnitXmlParserTest {
    ...
    void parsesCountsStatusesAndFailures() throws Exception {
        try (InputStream in = resource("/samples/surefire-sample.xml")) {
            ...
            ParsedCase failing = suite.cases().stream()
                    .filter(c -> c.name().equals("subtracts")).findFirst().orElseThrow();
            ...
            ParsedCase erroring = suite.cases().stream()
                    .filter(c -> c.name().equals("divides")).findFirst().orElseThrow();
