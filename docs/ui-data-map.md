...
| D2 | **Flag scope missing on Tests/Timing** | Solved free by D1 — the shared `scopeBar` flag dropdown appears on Tests/Timing/Load (hidden when the project has one flag). **Branch** dropdown ("All branches" + each branch) is now wired on **Tests/Timing** too (#431); Load/Coverage branch-scoping needs branch-aware perf/coverage queries — tracked in #431/#430. |
