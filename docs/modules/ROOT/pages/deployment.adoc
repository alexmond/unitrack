...
[source,bash]
----
podman login nas1.home.int:5000
./mvnw -pl unitrack-web -am package -DskipTests
podman build -t nas1.home.int:5000/unitrack:0.1.0-SNAPSHOT -f deploy/Containerfile unitrack-web/target
podman push --tls-verify=false nas1.home.int:5000/unitrack:0.1.0-SNAPSHOT
----
...
For the home k3s cluster (Zot registry, Traefik ingress at `unitrack.lab.alexmond.org`, node-local
