package io.github.alexmond.unitrack.web.ui;
...
public class DashboardController {
    ...
    public DashboardController(ReportingService reporting) {
    ...
    public String index(Model model) {
        ...
        for (Project p : projects) {
            ...
            reporting.recentRuns(p.getId(), 1).stream().findFirst()
                    .ifPresent(run -> latest.put(p.getId(), run));
        }
    ...
    public String project(@PathVariable Long id, Model model) {
        ...
        model.addAttribute("trendPassRate", toJson(trend.stream()
                .map(r -> round(r.passRate())).toList()));
        model.addAttribute("trendCoverage", toJson(trend.stream()
                .map(TestRun::getLineCoveragePct).toList()));
        return "project";
    }
    ...
    public String run(@PathVariable Long id, Model model) {
        ...
        List<?> files = coverage
                .map(c -> reporting.coverageFiles(c.getId(), COVERAGE_FILE_LIMIT))
                .orElse(List.of());
