package io.github.alexmond.unitrack.web.api;
...
/** JSON DTOs for the REST API. Kept separate from JPA entities so the wire format is stable. */
public final class ApiResponses {

    private ApiResponses() {
    }

    public record ProjectJson(Long id, String name, String repoUrl, long runCount) {
    ...
    public record RunJson(
            Long id, Long projectId, String branch, String commit, String buildUrl,
            Instant createdAt, int total, int passed, int failed, int errors, int skipped,
            long durationMs, String status, double passRate,
            Double lineCoveragePct, Double branchCoveragePct) {

        public static RunJson of(TestRun r) {
            return new RunJson(r.getId(), r.getProject().getId(), r.getBranch(), r.getCommitSha(),
                    r.getBuildUrl(), r.getCreatedAt(), r.getTotalTests(), r.getPassed(), r.getFailed(),
                    r.getErrors(), r.getSkipped(), r.getDurationMs(), r.getStatus(), r.passRate(),
                    r.getLineCoveragePct(), r.getBranchCoveragePct());
        }
    }
    ...
    public record SuiteJson(String name, int tests, int failures, int errors, int skipped, long durationMs) {
        public static SuiteJson of(TestSuiteResult s) {
            return new SuiteJson(s.getName(), s.getTests(), s.getFailures(), s.getErrors(),
                    s.getSkipped(), s.getDurationMs());
        }
    }

    public record CaseJson(String suite, String className, String name, String status,
                           long durationMs, String failureType, String failureMessage) {
    ...
    public record IngestResultJson(Long runId, Long projectId, String project, int total,
                                   int passed, int failed, int errors, int skipped, String status,
                                   Double lineCoveragePct) {
        public static IngestResultJson of(TestRun r) {
            return new IngestResultJson(r.getId(), r.getProject().getId(), r.getProject().getName(),
                    r.getTotalTests(), r.getPassed(), r.getFailed(), r.getErrors(), r.getSkipped(),
                    r.getStatus(), r.getLineCoveragePct());
        }
    }
}
