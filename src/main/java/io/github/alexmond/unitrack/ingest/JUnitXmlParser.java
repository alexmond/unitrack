...
public class JUnitXmlParser {
    ...
    public JUnitResults parse(InputStream in) {
        try {
            ...
            throw new IngestException("Failed to parse JUnit XML: " + e.getMessage(), e);
        }
    }
    ...
    private ParsedCase parseCase(String suiteName, Element caseEl) {
        ...
        if (!skipped.isEmpty()) {
            return new ParsedCase(suiteName, className, name, TestStatus.SKIPPED, durationMs,
                    null, null, null);
        }
        return new ParsedCase(suiteName, className, name, TestStatus.PASSED, durationMs,
                null, null, null);
    }

    private ParsedCase failureCase(String suiteName, String className, String name, long durationMs,
                                   TestStatus status, Element detail) {
